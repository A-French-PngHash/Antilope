...
def claim_tiles(user : User, activity : Activity, tiles : np.array):
    ...
    query_tile = """
    INSERT OR IGNORE INTO api_tile (x, y, user_id, last_update, nb_passage)
    VALUES (%s, %s, %s, %s, 0)"""
    ...
    query_update = """ UPDATE api_tile SET nb_passage = nb_passage + 1, last_update = %s WHERE x = %s AND y = %s;"""
    query_update_list = [(datetime.now(timezone.utc), int(tile[0]), int(tile[1])) for tile in tiles]
